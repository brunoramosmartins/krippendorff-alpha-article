...
> **What this is.** An agreement score of 0.80 can be misleading when chance agreement is not explicitly modelled. Under class imbalance and independent marginals, high observed overlap emerges without shared understanding. This article traces the statistical foundations of inter-annotator agreement — from raw observed agreement, through the Kappa family and where it breaks, to Krippendorff's Alpha, which models **disagreement** under a randomness benchmark and generalises across raters, missing data, and measurement scales. It grew out of a workplace project where a panel of humans disagreed with each other while an LLM was perfectly consistent — and the tempting conclusion "trust the model, it's consistent" turned out to be statistically fragile.
>
> **What you should know before reading.** Basic probability (distributions, expectation) and comfort with summation and matrix notation. No prior exposure to agreement coefficients is assumed — every one is built from scratch.
>
> **What you will take away.** Why raw agreement misleads under imbalance, how Cohen's and Fleiss' Kappa correct for chance and where they fail, and how Krippendorff's Alpha unifies the picture across raters, missing data, and measurement scales — plus a decision procedure for which coefficient to report.
>
> **Code.** The coefficients, simulations, and figures reproduce from the [companion repository](https://github.com/brunoramosmartins/krippendorff-alpha-article) with fixed seeds.
>
...
### Global agreement: pooling judgments with replacement
...
### What $A_o$ estimates — and what it misses
...
Like any binomial-style proportion, $A_o$ has **sampling variability**. Wide items and sparse rare classes can make point estimates noisy; the experiments below use $n = 10\,000$ items partly to make curves visually stable. In applied work, complement point estimates with **confidence intervals** (bootstrap over items is common) and with **disaggregated** views (per-stratum agreement, confusion matrices).
...
**Model 1 — Pure random labelling (no signal).** Each annotator draws $X_{ij} \sim \pi$ independently. There is no latent truth per item; agreement arises **only** from shared marginals. This model is the null hypothesis for the chance-agreement analysis and Experiment A.
...
The distinction matters: under Model 1, **any** observed agreement is pure chance. Under Model 2, agreement decomposes into a **signal** component (shared latent truth) and a **chance** component (marginal overlap). The coefficients in this article are designed to strip away the chance component — but they assume noise is **symmetric and item-independent**. Structured errors (e.g. an LLM that systematically over-predicts the majority class) violate this assumption and require additional diagnostic tools beyond what $\alpha$ alone provides (see the discussion of reliability versus validity below).

---

## The chance agreement problem

### Expected agreement under independence

**Model.** Two annotators assign labels **independently**, each following the same category distribution $\pi = (\pi_1,\ldots,\pi_K)$, $\pi_k \ge 0$, $\sum_k \pi_k = 1$ (Model 1 above). Then
...
### Random annotators: why $A_o$ does not go to zero
...
### Cohen's $\kappa$ (two fixed raters)
...
### Fleiss' $\kappa$ (several raters per item)
...
**Expected agreement:** $\bar P_e = \sum_k p_k^2$ (same functional form as the chance-agreement baseline when all judgments share one marginal).
...
Both $\kappa_C$ and $\kappa_F$ **re-express** overlap relative to a **data-driven** chance baseline. When $A_o$ is high only because $\bar P_e$ is high, $\kappa_F$ moves toward zero, signalling that apparent harmony is largely **prevalence-driven**. That is the right direction for interpretation — until the baseline and structural assumptions themselves become inadequate, as the next section shows.
...
### Building the coincidence matrix — a worked example
...
### From coincidence to disagreement
...
The conceptual shift is simple: instead of "how often do we agree?", ask **"how much farther apart are we than random pairing would predict?"** Agreement is the special case in which distance is zero on the diagonal and one elsewhere (nominal $\delta$). The next section assembles these pieces into the full $\alpha$ formula.
...
### Ordinal and interval scales in practice
...
Four simulations isolate properties of $A_o$, $\kappa_F$, and $\alpha$. All use **fixed seeds** and are reproducible via `make experiments` or the individual `scripts/experiment_*.py` commands.
...
### Experiment A: random annotators (sanity check)
...
### Experiment C: LLM versus humans (synthetic)
...
Walk the design top-down:

- **Missing ratings, or a varying number of raters per unit?** → **Krippendorff's $\alpha$**, choosing the level (nominal, ordinal, interval, ratio) that matches the scale. This is the common case in crowdsourcing and LLM–human panels.
- Otherwise, **exactly two fixed raters** on complete data? → **Cohen's $\kappa$**.
- Otherwise, the **same number of raters on every item** and a complete grid? → **Fleiss' $\kappa$**.
- Anything else (unequal raters, gaps) falls back to **$\alpha$**.

Whichever you land on, report it with a clear value domain and distance function, and put $A_o$ alongside it — plus a Kappa where valid — so a reader can see the raw overlap and the chance-corrected number in the same table.
...
### Reliability is not validity (the systematic bias problem)
...
### What agreement coefficients cannot fix
...
- Krippendorff, K. (2011). Computing Krippendorff's Alpha-Reliability. *Departmental Papers (ASC)*, University of Pennsylvania. [Available online](https://repository.upenn.edu/asc_papers/43/)
...
*The coefficients (`krippendorff_alpha()`, Cohen's and Fleiss' $\kappa$, observed agreement) and all four simulations are implemented and unit-tested in the [companion repository](https://github.com/brunoramosmartins/krippendorff-alpha-article), validated against published examples and the reference `krippendorff` Python package, and reproduce every figure with fixed seeds.*
